...
export async function POST({ request, locals }) {
    try {
        const formData = await request.formData();
        let emailContent = "";
        let subjectLine = formData.get("formSubject") || "Contact Form Submission";

        // Extracting fields and appending only if they have data
        for (const [key, value] of formData) {
            if (value && key !== "formSubject") {  // Exclude the formSubject from the body
        ...
        <html>
            <body class="font-sans bg-gray-500">
                <div class="max-w-2xl mx-auto p-4 bg-white shadow-md rounded-md mt-4">
                    <h2 class="text-2xl font-semibold mb-2">New ${subjectLine} Submission</h2>
                    <pre class="bg-gray-200 p-2 rounded-md">${emailContent.replace(/\n/g, '<br>')}</pre>
                </div>
            </body>
        </html>
        `;
